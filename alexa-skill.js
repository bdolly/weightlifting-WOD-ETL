...
const GetNewFactHandler = {
    ...
    async handle(handlerInput) {
        ...
        const workout = await getWorkout(day);
        // const randomFact = await getFact(locale, Math.floor(Math.random() * factCount));
        let speakOutput;
        if(workout)
            speakOutput = handlerInput.t('GET_FACT_MESSAGE') + workout.session;
        else
...
const getParams = (query) => {
    return {
        ...
        Bucket: process.env.INVICTUS_BUCKET,
        // Key: s3Key,
        Key: '/weekly',
        ExpressionType: 'SQL',
        Expression: query,
...
const getWorkout = async (day) => {
    try {
        const baseLocale = locale.split('-')[0];
        // const query = `SELECT COUNT(*) FROM S3Object[*].${baseLocale}.facts[*]`;
        const query = `SELECT * FROM s3object s WHERE s."date" like '%${day}%'`;
        const params = getParams(query);
        return await getS3SelectData(params);
    } catch (e) {
        console.log('select error: ' + e);
